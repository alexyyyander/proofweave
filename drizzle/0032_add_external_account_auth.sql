...
-- splitting their contribution record.
...
CREATE UNIQUE INDEX `person_identities_provider_subject_idx` ON `person_identities` (`provider`,`provider_subject`);
--> statement-breakpoint
CREATE INDEX `person_identities_person_idx` ON `person_identities` (`person_id`);
--> statement-breakpoint
CREATE INDEX `person_identities_verified_email_idx` ON `person_identities` (`email_normalized`,`email_verified_at`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `app_sessions_person_active_idx` ON `app_sessions` (`person_id`,`revoked_at`,`expires_at`);
