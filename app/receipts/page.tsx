  ...
  return <div className="site-shell app-shell">
    ...
    <main id="main-content" tabIndex={-1} className="page-main receipts-main">
      ...
      <div id="receipt-index">
        {unavailable ? <section className="receipt-message"><p className="eyebrow">Index unavailable</p><p>Contribution receipt storage is temporarily unavailable. No unverified fallback data is shown.</p></section> : receipts.length === 0 ? <section className="receipt-message receipt-empty-state"><div><p className="eyebrow">No issued public records yet</p><h2>See the complete evidence shape before the first live Receipt arrives.</h2><p>Receipts appear here only after kernel evidence and the required independent attestations are signed and stored. The reference demo is clearly labeled and does not pretend to be a live network contribution.</p></div><Link className="button button-primary" href="/demo#verification-console">Inspect the reference Receipt <span aria-hidden="true">→</span></Link></section> : <section className="receipt-index-list" aria-label="Verified Contribution Receipts">{receipts.map((receipt) => <ReceiptIndexCard receipt={receipt} key={receipt.id} />)}</section>}
      </div>
    </main>
    <Footer />
  </div>;
