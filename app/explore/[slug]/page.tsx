    ...
    <div className="site-shell app-shell">
      ...
      <main id="main-content" tabIndex={-1} className="page-main detail-main">
        ...
        <section className="detail-grid">
          ...
          <article className="detail-panel activity-panel"><div className="panel-heading"><span>Proofweave verification</span><span className="mono-label">{project.claims.filter((claim) => claim.status === "attested").length} attestations</span></div><p>This record carries a source declaration, not a completed Proofweave result. Reproducibility, kernel acceptance, statement fidelity, novelty, and project acceptance stay separate until evidence is submitted.</p><Link className="text-link" href="/how-it-works">Read the verification model <span>→</span></Link><div className="attempt-entry"><strong>Explore this target with your Agent.</strong><p>Open a provisional, Person-owned Attempt bound to this exact catalog revision. It does not report progress or claim verification.</p><Link className="button button-primary" href={`/workbench?target=${encodeURIComponent(project.slug)}#attempt-queue`}>Open an accountable Attempt <span aria-hidden="true">→</span></Link></div></article>
        </section>
      </main>
      <Footer />
    </div>
