...
import { CuratedResearchSchemaUnavailableError, getCuratedResearchRepository, type PublicCuratedResearchRecord } from "@/db/repositories/curated-research";
import { MissingDatabaseBindingError } from "@/db";
import { getCurrentUser, signInPath } from "../../auth";
import { Footer, PageSectionNav, StatusStack } from "../../ui";
import { Header } from "../../header";
import { CreditMarketPanel } from "./CreditMarketPanel";
import { PriorWorkChain } from "./PriorWorkChain";
...
export default async function ConjecturePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!project) notFound();
  const [graph, user, curatedResearch] = await Promise.all([
    loadResearchGraph(project.id),
    getCurrentUser(),
    loadCuratedResearch(project.id),
  ]);
  ...
  return (
    <div className="site-shell app-shell">
      ...
      <main id="main-content" tabIndex={-1} className="page-main detail-main">
        ...
        <PageSectionNav label="Research record" links={[{ href: "#statements", label: "Statements & source" }, { href: "#research-chain", label: "Prior work chain" }, { href: "#credit-market", label: "Credit market" }, { href: "#research-graph", label: "Research graph" }, { href: `/workbench?target=${encodeURIComponent(project.slug)}#research-launcher`, label: "Start with my Agent" }]} />
        <section className="detail-grid" id="statements">
        ...
        <PriorWorkChain records={curatedResearch} problemSlug={project.slug} />
        <div id="credit-market"><CreditMarketPanel market={market} /></div>
...
async function loadCuratedResearch(problemRevisionId: string): Promise<readonly PublicCuratedResearchRecord[]> {
  try {
    return await getCuratedResearchRepository().findByProblemRevisionId(problemRevisionId);
  } catch (error) {
    if (error instanceof MissingDatabaseBindingError || error instanceof CuratedResearchSchemaUnavailableError) return [];
    throw error;
  }
}
