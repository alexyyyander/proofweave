...
export function MyWorkOverview({
  ...
  return <>
    ...
    <div className={localConnection ? "my-work-agent is-connected" : "my-work-agent"}>
      <span><i aria-hidden="true" />{localConnection ? `${localConnection.agentLabel} is connected` : "No local research Agent connected"}</span>
      <Link href={localConnection ? "/settings" : "/integrations#codex-beta"}>{localConnection ? "Manage Agent" : "Connect Agent"} <span aria-hidden="true">→</span></Link>
    </div>
