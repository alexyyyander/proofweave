...
export default function CatalogStandardPage() {
  return <div className="site-shell app-shell"><Header active="about" /><main id="main-content" tabIndex={-1} className="page-main reading-main catalog-reading-main">
    ...
    <ol className="catalog-reading-gates">{gates.map(([number, title, copy, output]) => <li key={number}><span>{number}</span><div><h2>{title}</h2><p>{copy}</p></div><strong>{output}</strong></li>)}</ol>
    <section className="catalog-state-ladder" aria-label="Catalog review states"><div><span>Source-pinned</span><p>Statement and environment are reproducible.</p></div><b aria-hidden="true">→</b><div><span>Frontier-reviewed</span><p>Prior work and status were checked as of a stated date.</p></div><b aria-hidden="true">→</b><div><span>Proofweave-attested</span><p>Specific formal and review claims carry independent evidence.</p></div></section>
    <section className="reading-note"><div><p className="eyebrow">Current boundary</p><h2>Source-pinned does not mean frontier-reviewed.</h2></div><p>Many records are source-pinned imports and explicitly remain frontier-unreviewed. Proofweave does not convert upstream labels into its own mathematical attestation.</p></section>
