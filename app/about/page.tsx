...
export default function AboutPage() {
  return <div className="site-shell app-shell"><Header active="about" /><main id="main-content" tabIndex={-1} className="page-main information-hub-main">
    ...
    <section className="information-path-grid about-topic-grid" id="design-documents" aria-label="Proofweave design documents">{topics.map((topic) => <Link href={topic.href} key={topic.href}><span>{topic.number}</span><h2>{topic.title}</h2><p>{topic.copy}</p><strong>Read this document <b aria-hidden="true">→</b></strong></Link>)}</section>
