...
export default function ContributionRecordsGuidePage() {
  return <div className="site-shell app-shell"><Header active="how" /><main id="main-content" tabIndex={-1} className="page-main reading-main">
    ...
    <section className="record-layer-list" aria-label="Contribution record layers">{records.map(([title, copy], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h2>{title}</h2><p>{copy}</p></article>)}</section>
    <section className="reading-note"><div><p className="eyebrow">Credit boundary</p><h2>Compute spend never mints mathematical credit.</h2></div><p>Non-transferable research credit follows policy-defined, evidence-bearing work and its dependency graph. Funding, compute, coordination, verification, and intellectual contribution remain separately labeled.</p></section>
