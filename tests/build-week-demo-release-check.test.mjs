...
test("the release check covers the public story, verifier, tamper path, and publication boundary", async () => {
  ...
  assert.equal(result.internalSmokeExcluded, true);
...
function createFetch({ tamperRejected }) {
  const responses = new Map([
    ...
    [`${baseUrl}/demo`, html("Watch one Lean proof become verifiable evidence. A real cloud replay produced an internal test Receipt. This is not a public math contribution or Research Credit")],
    ...
    [`${baseUrl}/api/receipts`, json({ receipts: [] })],
  ]);
