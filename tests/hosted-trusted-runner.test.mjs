...
test("hosted Runner exposes health and authenticated wake without accepting work", async (t) => {
  ...
  const service = new HostedTrustedRunnerService({
    ...
    runtimeFactory: async () => ({
      releaseDiagnostics: readyDiagnostics(),
      async run({ signal }) {
        runStarted = true;
  ...
  assert.deepEqual(healthBody.releaseDiagnostics, readyDiagnostics());
...
test("hosted Runner verifies Turso and stays healthy but non-executing while paused", async (t) => {
  ...
  const service = new HostedTrustedRunnerService({
    ...
    diagnosticsFactory: async () => readyDiagnostics(),
    runtimeFactory: async () => {
      runtimeStarted = true;
  ...
  assert.deepEqual(body.releaseDiagnostics, readyDiagnostics());
